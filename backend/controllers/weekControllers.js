...
const updateWeekData = async (req, res) => {
  ...
  try {
        ...
        if (weeklyData.days[day]) {
          if (updatedDays[day].minutesDoneToday !== undefined) {
            if (updatedDays[day].minutesDoneToday < 0) {
              return res
                .status(400)
                .json({ error: `Activity time for ${day} cannot be negative` });
            }
            weeklyData.days[day].minutesDoneToday =
              updatedDays[day].minutesDoneToday;
            weeklyData.days[day].dailyImbalance =
              weeklyData.days[day].minutesDoneToday - weeklyData.dailyGoal;
          }
        }
      }
    }

    const updatedMinutesDoneWeek = Object.values(weeklyData.days).reduce(
      (sum, day) => sum + day.minutesDoneToday,
      0
    );
    ...
    weeklyData.weeklyImbalance =
      weeklyData.minutesDoneWeek - weeklyData.minutesGoalWeek;
...
const getWeek = async (req, res) => {
  ...
  try {
    const userID = req.user._id;
    const week = await Week.findOne({ userID, weekID }).populate({
      path: "weekData",
    });
