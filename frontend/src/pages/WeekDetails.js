...
const WeekDetails = () => {
  ...
  const validateTime = (value) => {
    ...
    if (
      ...
      setError(
        "Invalid time. Please enter a valid time between 00:00 and 24:00."
      );
      return false;
    }
  ...
  const getImba = (minutes, wgoal) => {
    const dgoal = wgoal / 7;
    const imba = minutes - dgoal;

    const imbaPercentage = (100 / dgoal) * imba;
    ...
    const fetchWeek = async () => {
      try {
        const response = await fetch(
          `http://localhost:4000/api/weeks/id/${weekID}`,
          {
  ...
  return (
    <div className="main-content">
            ...
            <tr>
              ...
              {days.map((day) => (
                <th key={`${day}-date`} className="date">
                  {week?.days[day]
                    ? new Date(week.days[day]).toLocaleDateString("en-US", {
                        year: "numeric",
                        month: "numeric",
                        day: "numeric",
                      })
          ...
          <tbody>
            {week?.habits?.length > 0 &&
              week.habits.map((habit) => {
                const weeklyData = week.weekData.find(
                  (data) => data.habitID.toString() === habit._id.toString()
                );
                ...
                return (
                  <tr key={habit._id}>
                    ...
                    <td>
                      {!week.done && (
                        <input
                          ...
                          defaultValue={
                            weeklyData
                              ? formatTime(weeklyData.minutesGoalWeek)
                              : ""
                          }
                          onBlur={(e) => {
                    ...
                    <td>
                      {weeklyData ? formatTime(weeklyData.dailyGoal) : ""}
                    </td>
                    {days.map((day) => (
                      <td
                        className={
                          weeklyData
                            ? getImba(
                                weeklyData?.days[day].minutesDoneToday,
                                weeklyData?.minutesGoalWeek
                              )
                            : ""
                        }
                        key={day}
                    ...
                    <td
                      className={
                        weeklyData
                          ? getImba(
                              weeklyData.weeklyImbalance,
                              weeklyData.minutesGoalWeek
                            )
                          : ""
                      }
                    >
                      {weeklyData ? formatTime(weeklyData.weeklyImbalance) : ""}
                    </td>
                    <td>
                      {weeklyData ? formatTime(weeklyData.minutesDoneWeek) : ""}
                    </td>
                  </tr>
                );
              })}
      ...
      <div className="week-status">
        <button
          className={`status-button ${week.done ? "edit" : "done"}`}
          onClick={toggleDone}
        >
          {week.done ? "Edit" : "Done"}
        </button>
